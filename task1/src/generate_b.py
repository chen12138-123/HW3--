...
try:
    from .demo_pipeline import make_object_b, plot_cloud
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Object B: text-to-3D generation with threestudio SDS.")
    parser.add_argument("--prompt", type=str, required=True, help="Text prompt for text-to-3D generation.")
    parser.add_argument("--out", type=Path, required=True, help="Output directory.")
    parser.add_argument("--repo", type=Path, default=Path("submodules/threestudio/threestudio-main"))
    parser.add_argument("--config", type=str, default="configs/dreamfusion-sd.yaml")
...
def run_threestudio(args: argparse.Namespace) -> None:
...
def run_demo(args: argparse.Namespace) -> None:
    ...
    cloud = make_object_b(rng)
    args.out.mkdir(parents=True, exist_ok=True)
    save_xyz_rgb_ply(args.out / "model.ply", cloud.points, cloud.colors)
    plot_cloud(cloud, Path("figures/object_b.png"), "Object B: threestudio text-to-3D asset")
    print(f"Demo Object B saved to {args.out / 'model.ply'}")
